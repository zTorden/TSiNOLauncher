...
public class LegacyAuthenticationService extends BaseAuthenticationService {
	private static final URL AUTHENTICATION_URL = Util
			.constantURL("https://login.minecraft.net");
	...
	public void logIn() throws AuthenticationException {
		...
		String response;
		try {
			response = Http.performPost(AUTHENTICATION_URL, args,
					Launcher.getInstance().getProxy()).trim();
		} catch (IOException e) {
			throw new AuthenticationException(
					"Authentication server is not responding", e);
		}
		...
		if (split.length == 5) {
			...
			if ((StringUtils.isBlank(profileId))
				...
				throw new AuthenticationException(
						"Unknown response from authentication server: "
								+ response);
			}
	...
	public void selectGameProfile(GameProfile profile)
		...
		throw new UnsupportedOperationException(
				"Game profiles cannot be changed in the legacy authentication service");
	}
