...
public class CrashReportTab extends JPanel {
	...
	public CrashReportTab(final Launcher launcher,
			final CompleteVersion version, File reportFile, final String report) {
		super(true);
		...
		if ((report.contains("Is Modded: Probably not"))
				|| (report.contains("Is Modded: Unknown")))
			this.isModded = (!report
					.contains("Suspicious classes: No suspicious classes found."));
		else {
			this.isModded = true;
		}
		...
		if (launcher.getProfileManager().getSelectedProfile()
				.getUseHopperCrashService())
			launcher.getVersionManager().getExecutorService()
					.submit(new Runnable() {
						public void run() {
							try {
								Map<String, String> environment = new HashMap<String, String>();
								environment.put("launcher.version", "1.2.3");
								environment.put("launcher.title", launcher
										.getFrame().getTitle());
								environment.put("bootstrap.version",
										String.valueOf(launcher
												.getBootstrapVersion()));
								CrashReportTab.this.hopperServiceResponse = HopperService
										.submitReport(launcher.getProxy(),
												report, "Minecraft",
												version.getId(), environment);
								launcher.println("Reported crash to Mojang (ID "
										+ CrashReportTab.this.hopperServiceResponse
												.getReport().getId() + ")");

								if (CrashReportTab.this.hopperServiceResponse
										.getProblem() != null)
									CrashReportTab.this.showKnownProblemPopup();
								else if (CrashReportTab.this.hopperServiceResponse
										.getReport().canBePublished())
									CrashReportTab.this
											.showPublishReportPrompt();
							} catch (IOException e) {
								launcher.println(
										"Couldn't report crash to Mojang", e);
							}
						}
					});
	}
	...
	private void showPublishReportPrompt() {
		...
		message.setText("<html><p>Sorry, but it looks like the game crashed and we don't know why.</p><p>Would you mind publishing this report so that "
				+ (this.isModded ? "the mod authors" : "Mojang")
				+ " can fix it?</p></html>");

		int result = JOptionPane.showOptionDialog(this, message,
				"Uhoh, something went wrong!", 0, 1, null, options, options[0]);

		if (result == 0)
			try {
				PublishResponse localPublishResponse = HopperService
						.publishReport(this.launcher.getProxy(),
								this.hopperServiceResponse.getReport());
			} catch (IOException e) {
				this.launcher.println("Couldn't publish report "
						+ this.hopperServiceResponse.getReport().getId(), e);
			}
	}

	private void showKnownProblemPopup() {
		if (this.hopperServiceResponse.getProblem().getUrl() == null) {
			JOptionPane.showMessageDialog(this, this.hopperServiceResponse
					.getProblem().getDescription(), this.hopperServiceResponse
					.getProblem().getTitle(), 1);
		} else {
			String[] options = { "Fix The Problem", "Cancel" };
			int result = JOptionPane.showOptionDialog(this,
					this.hopperServiceResponse.getProblem().getDescription(),
					this.hopperServiceResponse.getProblem().getTitle(), 0, 1,
					null, options, options[0]);

			if (result == 0)
				try {
					OperatingSystem.openLink(new URI(this.hopperServiceResponse
							.getProblem().getUrl()));
				} catch (URISyntaxException e) {
					this.launcher.println("Couldn't open help page ( "
							+ this.hopperServiceResponse.getProblem().getUrl()
							+ "  ) for crash", e);
				}
		}
	}
	...
	private class CrashInfoPane extends JPanel implements ActionListener {
		...
		protected void createInterface() {
			...
			add(new JLabel(
					CrashReportTab.this.isModded ? "<html><div style='width: 100%'><p><b>Uhoh, it looks like the game has crashed! Sorry for the inconvenience :(</b></p><p>We think your game may be modded, and as such we can't accept this crash report.</p><p>However, if you do indeed use mods, please send this to the mod authors to take a look at!</p></div></html>"
							: "<html><div style='width: 100%'><p><b>Uhoh, it looks like the game has crashed! Sorry for the inconvenience :(</b></p><p>Using magic and love, we've managed to gather some details about the crash and we will investigate this as soon as we can.</p><p>You can see the full report below.</p></div></html>"),
					constraints);
			...
			if (e.getSource() == this.submitButton) {
				if (CrashReportTab.this.hopperServiceResponse != null) {
					...
					try {
						...
						OperatingSystem
								.openLink(URI
										.create("https://mojang.atlassian.net/secure/CreateIssueDetails!init.jspa?"
												+ Http.buildQuery(args)));
					} catch (Throwable ex) {
						Launcher.getInstance().println(
								"Couldn't open bugtracker", ex);
					}
			} else if (e.getSource() == this.openFileButton)
				OperatingSystem
						.openLink(CrashReportTab.this.reportFile.toURI());
		}
 ...
 * net.minecraft.launcher.ui.tabs.CrashReportTab JD-Core Version: 0.6.2
 */