...
public class ProfileInfoPanel extends JPanel {
	...
	private final JCheckBox useHopper = new JCheckBox(
			"Automatically ask Mojang for assistance with fixing crashes");
	private final JCheckBox launcherVisibilityCustom = new JCheckBox(
			"Launcher Visibility:");
	...
	protected void fillDefaultValues() {
		...
		if (gameDir != null) {
			...
			this.gameDirField.setText(this.editor.getLauncher()
					.getWorkingDirectory().getAbsolutePath());
		}
		updateGameDirState();

		Profile.Resolution resolution = this.editor.getProfile()
				.getResolution();
		...
		this.useHopper.setSelected(this.editor.getProfile()
				.getUseHopperCrashService());

		LauncherVisibilityRule visibility = this.editor.getProfile()
				.getLauncherVisibilityOnGameClose();
		...
		if (visibility != null) {
			...
			this.launcherVisibilityOption
					.setSelectedItem(Profile.DEFAULT_LAUNCHER_VISIBILITY);
		}
	...
	private void updateLauncherVisibilityState() {
		...
		if ((this.launcherVisibilityCustom.isSelected())
				&& ((this.launcherVisibilityOption.getSelectedItem() instanceof LauncherVisibilityRule))) {
	...
	private void updateResolution() {
		try {
			...
			this.editor.getProfile().setResolution(
					new Profile.Resolution(width, height));
 ...
 * net.minecraft.launcher.ui.popups.profile.ProfileInfoPanel JD-Core Version:
 * 0.6.2
 */